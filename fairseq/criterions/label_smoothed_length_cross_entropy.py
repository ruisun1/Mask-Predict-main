...
class LabelSmoothedLengthCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        #hypo = torch.argmax(lprobs, dim =1)
        #print(hypo)
        #hypo = hypo.view(lprobs(0),lprobs(1))
        #print(hypo)
        index = torch.max(lprobs,dim = -1)[1]
       ...
       # length_loss = length_loss.new(length_loss.size()).fill_(0)#jiiaru
        if reduce:
...
#            length_loss = length_loss.sum()
        ...
        return loss, nll_loss, loss, non_pad_mask.sum().data.item()
