...
@register_criterion('confusion_loss2')
class LabelSmoothedLengthCrossEntropyCriterionCFL2(FairseqCriterion):

    def __init__(self, args, task):
        super().__init__(args, task)
        self.eps = args.label_smoothing
        f = open('/home/sunrui/Mask-Predict-main/fairseq/criterions/SoundConfusion.json')
        dict = json.load(f)
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
       ...
       #     confusion_line = torch.cat((confusion_line, torch.tensor(self.confusion_matrix[word])), dim = 0)
        ...
        probs = torch.pow(e, lprobs_nll)
        sort_probs = (probs*confusion_line)[non_pad_mask.squeeze(1)]
        #print(probs[non_pad_mask.squeeze(1)])
        #probs_5 , _ = torch.sort( sort_probs, descending = True, dim = -1)
        #print(probs_5[non_pad_mask.squeeze(1)])
        #probs_5 = probs_5[:,:5]
        confusion_probs =  -torch.sum( lprobs_nll *confusion_line, dim = -1).unsqueeze(1)[non_pad_mask]
        #print("prob5 sum",confusion_probs)
        #confusion_probs = torch.pow(e, confusion_probs)
        l_probs = -lprobs_nll.gather(dim = -1, index= target_nll)[non_pad_mask]
        #l_probs = torch.pow(e, l_probs)
        confusion_probs = confusion_probs+0.1
        confusion_num = torch.sum(confusion_line, dim = -1).unsqueeze(1)[non_pad_mask]
        #print("lprobs",l_probs)
        #print("conprobs",confusion_probs)
        #print(l_probs.size())
       ...
       # print("confusioon_probs", confusion_probs)
        # loss_cf = torch.sum(torch.tan(l_probs/(confusion_probs + l_probs)))
       # loss_cf = -torch.sum(torch.tanh(2*l_probs/(confusion_probs + l_probs)))
        lambda_1 = 1
        loss = loss_nll - lambda_1 * loss_cf
