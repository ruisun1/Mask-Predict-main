...
class LanguagePairSelfDatasetMask(FairseqDataset):
    ...
    def __init__(
        ...
        self.z = z
        self.msk = msk
        self.msk_sizes = np.array(msk_sizes)
        ...
        self.seed = seed
        #print(self.src_sizes,self.z_sizes, self.tgt_sizes, self.msk_sizes)
        #print("language paie dataset",src_dict.mask(),tgt_dict.mask())
    def __getitem__(self, index):
        #print(self.src_dict.string(self.src[index]))
        #print(self.src_dict.string(self.msk[index]))
        enc_x_source, enc_z_source, dec_source, dec_target, ntokens = self._make_source_target(self.src[index], self.z[index],self.tgt[index], self.msk[index])
    ...
    def _make_source_target(self, source_x, source_z, target, msk):
...
#        self.mask_range = True
        ...
        if self.train:
            ...
            dec_source = msk
        for i in range(len(dec_source)):
            dec_source[i] = dec_source[i] #if dec_source[i]!=5713 else 4
        ntokens = dec_target.ne(self.tgt_dict.pad()).sum(-1).item()
        #print ("masked tokens", dec_source)
        #print ("original tokens", self.tgt_dict.string(dec_target))
        #print ("original tokens", dec_target)
        #print ("source tokens", self.src_dict.string(enc_x_source))
    ...
    def size(self, index):
        ...
        return (self.src_sizes[index], self.z_sizes[index], self.tgt_sizes[index], self.msk_sizes[index])
    ...
    def ordered_indices(self):
       ...
       # print(len(self.tgt_sizes),len(self.z_sizes),len(self.src_sizes))
        ...
        if self.msk_sizes is not None:
            indices = indices[np.argsort(self.msk_sizes[indices], kind='mergesort')]
    ...
    def supports_prefetch(self):
        return (
            ...
            and hasattr(self.msk, 'supports_prefetch')
            and self.msk.supports_prefetch)
