...
class Dictionary(object):
    ...
    def finalize(self, threshold=-1, nwords=-1, padding_factor=8):
        ...
        for symbol, count in c.most_common(nwords - self.nspecial):
            if count >= threshold:
                #print(symbol, new_indices[symbol])
                new_indices[symbol] = len(new_symbols)
                new_symbols.append(symbol)
                new_count.append(new_indices[symbol])
                #print(symbol, new_indices[symbol])
            else:
                break
        ...
        print(self.unk_index, self.mask_index)
    ...
    def save(self, f):
        ...
        self._save(f, zip(self.symbols[self.nspecial:],  self.count[self.nspecial:]))
    ...
    @staticmethod
    def add_file_to_dictionary(filename, dicti, tokenize, num_workers):
        def merge_result(counter):
            for w, c in counter.items():
                dicti.add_symbol(w, c)
        ...
        if num_workers > 1:
            ...
            for worker_id in range(num_workers):
                results.append(pool.apply_async(
                    ...
                    (filename, tokenize, dicti.eos_word, worker_id, num_workers)
                ))
            pool.close()
            pool.join()
            for r in results:
            ...
            merge_result(Dictionary._add_file_to_dictionary_single_worker(filename, tokenize, dicti.eos_word))
