...
class Dictionary2(object):
    ...
    def __init__(
        ...
        for line in lines:
            line=line[:-1]
            word, indice = line.split(' ')
            indice = int(indice)
            self.indices[word]=indice
            self.symbols.append(word)
            self.nums.append(indice)
            self.word_idx[indice]=word
            if word == unk:
                self.unk_index=(indice)
            elif word == pad:
                self.pad_index=(indice)
            elif word == mask:
                self.mask_index=indice
            elif word == eos:
                self.eos_index= indice
            elif word == bos:
                self.bos_index= indice
        self.unk_word, self.pad_word, self.eos_word, self.mask_word = unk, pad, eos, mask
        self.nspecial = len(self.symbols)
        self.unk_index, self.pad_index, self.eos_index, self.mask_index = 3,1,2,4
        print("word_idx",self.word_idx)
    ...
    def string(self, tensor, bpe_symbol=None, escape_unk=False):
        ...
        def token_string(i):
            if i == self.unk():
                ...
                return self.eos_word
            elif i == self.mask():
                return self.mask_word
            else:
                return self.word_idx[i.item()]
    ...
    def save(self, f):
        ...
        self._save(f, zip(self.symbols,self.nums))
