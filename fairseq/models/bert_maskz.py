...
class Transformer_nonautoregressive(FairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        encoder = TransformerEncoder(args, src_dict, encoder_embed_tokens, bert, args.encoder_embed_scale)
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
       ...
       # segments_ids=prev_output_tokens.new(prev_output_tokens.size())._fill(0)
        ...
        for layer in self.layers:
            x, attn = layer(
                x,
                encoder_out['encoder_x_out'] if encoder_out is not None else None,
                encoder_out['encoder_x_padding_mask'] if encoder_out is not None else None,
                encoder_out['encoder_z_out'] if encoder_out is not None else None,
                encoder_out['encoder_x_attention_mask'] if encoder_out is not None else None,
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_x_out, encoder_x_padding_mask, encoder_z_out, encoder_x_attention_mask, decoder_padding_mask):
        """
        Args:
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(self, srcx_tokens, srcx_lengths , srcz_tokens):
        """
        Args:
        ...
        atten_mask = []
        for bsz in range(srcz_tokens.size(0)):
            m = srcz_tokens[bsz].ne(self.masking_idx)
            mk1 = m*srcz_tokens.size(1)
            mk1 = np.array(mk1.cpu()).reshape(srcz_tokens.size(1),-1)
            mk2 = mk1.T
            atten_mask.append( mk1 * mk2)
        atten_mask = torch.FloatTensor(atten_mask)
        atten_mask = atten_mask.repeat(self.encoder_attention_heads, 1, 1).cuda()
        #print("atten_mask",atten_mask.size())
        if not encoder_padding_mask.any():
            encoder_padding_mask = None
        #segments_ids=srcx_tokens.new(srcx_tokens.size()).fill_(0)
        # encoder layer
        #print(srcx_tokens.type)
...
#        print("x size",x.size())
        ...
        for layer in self.layers_z:
            x2 = layer(x, encoder_padding_mask, atten_mask)
        if self.normalize:
       ...
       #     encoder_padding_mask = encoder_padding_mask[:, 1:]
        ...
        return {
            'encoder_x_out': x1,  # T x B x C
            'encoder_z_out': x2,
            'encoder_x_padding_mask': encoder_padding_mask,  # B x T
            'encoder_x_attention_mask': atten_mask,
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_padding_mask , atten_mask = None):
        """
        Args:
            x (Tensor): input to the layer of shape `(seq_len, batch, embed_dim)`
            encoder_padding_mask (ByteTensor): binary ByteTensor of shape
        ...
        #print("x size",x.size())
        x, _ = self.self_attn(query=x, key=x, value=x, key_padding_mask=encoder_padding_mask , attn_mask = atten_mask)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
...
@register_model_architecture('bert_maskz', 'bert_maskz')   #args是一个对象，getattr用来获取对象中的属性值
...
@register_model_architecture('bert_maskz', 'bert_maskz_big')
