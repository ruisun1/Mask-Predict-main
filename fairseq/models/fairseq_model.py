...
class FairseqEncoderDecoderModel_ori(BaseFairseqModel):
     ...
     def forward(self, srcx_tokens, srcx_lengths,  prev_output_tokens, **kwargs):
        """
        ...
        """
        encoder_out = self.encoder(srcx_tokens = srcx_tokens, srcx_lengths= srcx_lengths, **kwargs)
     ...
     def extract_features(self, src_tokens, src_lengths, prev_output_tokens, **kwargs):
        ...
        Returns:
            tuple:
                ...
                - a dictionary with any model-specific outputs
        """
        encoder_out = self.encoder(srcx_tokens, src_lengths=src_lengths, **kwargs)
...
class FairseqEncoderDecoderModel(BaseFairseqModel):
    """Base class for encoder-decoder models.
        this is a class for guided seq2seq

        with 2 encoder
    Args:
        encoderx (FairseqEncoder): the encoder
        encoderz (FairseqEncoder): the encoder
    ...
    def __init__(self, encoderx, encoderz, decoder):
        ...
        self.decoder = decoder
        assert isinstance(self.encoderx, FairseqEncoder)
        assert isinstance(self.encoderz, FairseqEncoder)
        assert isinstance(self.decoder, FairseqDecoder)

    def forward(self, srcx_tokens, srcx_lengths, srcz_tokens, srcz_lengths, prev_output_tokens, **kwargs):
        """
        Run the forward pass for an encoder-decoder model.
        ...
        Returns:
            tuple:
                ...
                - a dictionary with any model-specific outputs
        """
        encoderx_out = self.encoderx(srcx_tokens, srcx_lengths=srcx_lengths, **kwargs)
        encoderz_out = self.encoderz(srcz_tokens, srcz_lengths=srcz_lengths, **kwargs)
        decoder_out = self.decoder(prev_output_tokens, encoder_x_out=encoderx_out, encoder_z_out=encoderz_out, **kwargs)
        return decoder_out
    ...
    def max_positions(self):
        """Maximum length supported by the model."""
        return (self.encoderx.max_positions(), self.decoder.max_positions())
