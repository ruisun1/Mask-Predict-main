...
class pinyinEmbedding:
    def __init__(self, embed_dim , padding_idx):
        ...
        self.tokenid_pyid = {}
        f=open('./pinyin_data/dict.cor.txt','r',encoding='utf-8')
        lines=f.readlines()
        self.vocab={}
        self.index={}
        self.tokenid_pyid = {}
    ...
    def encode_batch(self, inputer):
        ...
        for bsz in inputer:
            ...
            for idx in bsz:
#                print(self.index)
 #               print(id.item(),self.index[id.item()]
                ids.append(self.tokenid_pyid[self.index[idx.item()]])
                output.append(self.tokenid_pyid[self.index[idx.item()]])
           # print(bsz, ids)
        output = np.array(output)
        output = output.reshape(inputer.size())
       # print("encode", output.size())
    ...
    def make_embedding(self, inputer):
        ...
        emb = nn.Embedding(num_vocab, self.embed_dim, self.padding_idx)
        embed_py = emb(encode)
        return embed_py
...
@register_model('bert_transformer_seq2seq_pinyin')
class Transformer_nonautoregressive(FairseqModel):
    def __init__(self, encoderx, encoderz,  decoder):
        super().__init__(encoderx, encoderz, decoder)
        self.apply(self.init_bert_weights)

    def init_bert_weights(self, module):  # 根据module 赋予weights
    ...
    def build_model(cls, args, task):
        ...
        def build_embedding(dictionary, embed_dim, is_encoder, path=None):
            ...
            embed_pinyin = pinyinEmbedding(embed_dim, padding_idx )
            if path:
                embed_dict = utils.parse_embedding(path)
                utils.load_embedding(embed_dict, dictionary, emb)
            return emb, embed_pinyin
        ...
        if args.share_all_embeddings:
            ...
            encoder_embed_tokens , encoder_embed_pinyin= build_embedding(
                src_dict, args.encoder_embed_dim, is_encoder=True, path=args.encoder_embed_path
            )
            decoder_embed_tokens = encoder_embed_tokens
            decoder_embed_pinyin = encoder_embed_pinyin
            ...
            decoder_embed_tokens, decoder_embed_pinyin = build_embedding(
                tgt_dict, args.decoder_embed_dim, is_encoder=False, path=args.decoder_embed_path
            )



        encoder_x = TransformerEncoder_x(args, src_dict, encoder_embed_tokens, encoder_embed_pinyin, args.encoder_embed_scale)
        encoder_z = TransformerEncoder_z(args, src_dict, encoder_embed_tokens, encoder_embed_pinyin, args.encoder_embed_scale)
        decoder = SelfTransformerDecoder(args, tgt_dict, decoder_embed_tokens, decoder_embed_pinyin, args.decoder_embed_scale)
        return Transformer_nonautoregressive(encoder_x,encoder_z, decoder)
...
class SelfTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, embed_pinyin, embed_scale=None, no_encoder_attn=False, left_pad=False,
                 final_norm=True):
        ...
        self.embed_positions = PositionalEmbedding(       # fairseq LearnedPositionalEmbedding
    ...
    def forward(self, prev_output_tokens, encoder_x_out=None, encoder_z_out=None, incremental_state=None):
        """
        Args:
       ...
       # print("decoder_padding_mask",decoder_padding_mask.size())
        # embed positions
        positions = self.embed_positions(
            prev_output_tokens,
        ) if self.embed_positions is not None else None
        pinyin = self.embed_pinyin.make_embedding(prev_output_tokens).to(prev_output_tokens.device)
        # embed tokens and positions
        x = self.embed_tokens(prev_output_tokens)   # from build embeddings
        ...
        for layer in self.layers:
            x, attn = layer(
                x,
                encoder_x_out['encoder_x_out'] if encoder_x_out is not None else None,
                encoder_x_out['encoder_x_padding_mask'] if encoder_x_out is not None else None,
                encoder_z_out['encoder_z_out'] if encoder_z_out is not None else None,
                encoder_z_out['encoder_z_padding_mask'] if encoder_z_out is not None else None,
        ...
        return x, {'attn': attn, 'inner_states': inner_states, 'predicted_lengths': encoder_x_out['predicted_lengths']}
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False):
        super().__init__()
        self.embed_dim = args.decoder_embed_dim
        self.self_attn = MultiheadAttention(      # fairseq MultiheadAttention
    ...
    def forward(self, x, encoder_x_out, encoder_x_padding_mask, encoder_z_out, encoder_z_padding_mask, decoder_padding_mask):
        """
        Args:
        ...
        if self.encoder_attn is not None:
            ...
            x, attn = self.encoder_attn(
                ...
                key=encoder_x_out,
                value=encoder_x_out,
                key_padding_mask=encoder_x_padding_mask,
                static_kv=True,
            ...
            x, attn = self.encoder_attn(
                ...
                key=encoder_z_out,
                value=encoder_z_out,
                key_padding_mask=encoder_z_padding_mask,
                static_kv=True,
                need_weights=(not self.training and self.need_attn),
            )
            x = F.dropout(x, p=self.dropout, training=self.training)
...
class TransformerEncoder_x(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, embed_pinyin, embed_scale=None, left_pad=False):
        super().__init__(dictionary)
        self.dropout = args.dropout
        ...
        self.embed_pinyin = pinyinEmbedding(
        ...
        self.layers.extend([
            ...
            for i in range(args.encoder_layers_x)
        ])
        self.register_buffer('version', torch.Tensor([2]))
    ...
    def forward(self, srcx_tokens, srcx_lengths):
        ...
        """
        print(srcx_tokens)
        #print("src_tokens size",srcx_tokens)
        # embed tokens and positions
        x = self.embed_tokens(srcx_tokens)
        #assert (src_tokens.size(1) < self.embed_positions.weights.data.size(0))
        if self.embed_positions is not None:
            x = x + self.embed_positions(srcx_tokens)
       
        pinyin = self.embed_pinyin.make_embedding(srcx_tokens).to(x.device)
       #print("x:",x.size())
        #print("pinyin:",pinyin.size())
        #print("position:", self.embed_positions(srcx_tokens).size())
        x += pinyin
        #len_tokens = self.embed_lengths(src_tokens.ne(self.padding_idx).sum(-1).unsqueeze(-1))   # If enabled, input of len token is src len
        len_tokens = self.embed_lengths(srcx_tokens.new(srcx_tokens.size(0), 1).fill_(0))
        ...
        encoder_padding_mask = srcx_tokens.eq(self.padding_idx)
        encoder_padding_mask = torch.cat([encoder_padding_mask.new(srcx_tokens.size(0), 1).fill_(0), encoder_padding_mask], dim=1)
        ...
        return {
            'encoder_x_out': x,  # T x B x C
            'encoder_x_padding_mask': encoder_padding_mask,  # B x T
    ...
    def reorder_encoder_out(self, encoder_out, new_order):
        ...
        """
        if encoder_out['encoder_x_out'] is not None:
            encoder_out['encoder_x_out'] = \
                encoder_out['encoder_x_out'].index_select(1, new_order)
        if encoder_out['encoder_x_padding_mask'] is not None:
            encoder_out['encoder_x_padding_mask'] = \
                encoder_out['encoder_x_padding_mask'].index_select(0, new_order)
...
class TransformerEncoder_z(FairseqEncoder):
    """
    Transformer encoder consisting of *args.encoder_layers* layers. Each layer
    is a :class:`TransformerEncoderLayer`.
    Args:
    ...
    def __init__(self, args, dictionary, embed_tokens, embed_pinyin,  embed_scale=None, left_pad=False):
     ...
     #       left_pad=left_pad,
      #      learned=args.encoder_learned_pos,
        ...
        self.embed_lengths = nn.Embedding(args.max_target_positions, self.embed_dim)
        nn.init.normal_(self.embed_lengths.weight, mean=0, std=0.02)

        self.layers = nn.ModuleList([])
        self.layers.extend([
            TransformerEncoderLayer(args)
            for i in range(args.encoder_layers_z)
        ])
        self.register_buffer('version', torch.Tensor([2]))
        self.normalize = args.encoder_normalize_before
        if self.normalize:
    ...
    def forward(self, srcz_tokens, srcz_lengths):
        """
        Args:
            src_tokens (LongTensor): tokens in the source language of shape
                `(batch, src_len)`
        ...
        Returns:
            dict:
                ...
                - **encoder_padding_mask** (ByteTensor): the positions of
                  padding elements of shape `(batch, src_len)`
        """
        #print("srcz tokens size",srcz_tokens)
        # embed tokens and positions
        x = self.embed_tokens(srcz_tokens)
        #assert (src_tokens.size(1) < self.embed_positions.weights.data.size(0))
        if self.embed_positions is not None:
            x = x + self.embed_positions(srcz_tokens)
        pinyin = self.embed_pinyin.make_embedding(srcz_tokens).to(x.device)
        x += pinyin
        #len_tokens = self.embed_lengths(src_tokens.ne(self.padding_idx).sum(-1).unsqueeze(-1))   # If enabled, input of len token is src len
        len_tokens = self.embed_lengths(srcz_tokens.new(srcz_tokens.size(0), 1).fill_(0))
        x = torch.cat([len_tokens, x], dim=1)
        x = F.dropout(x, p=self.dropout, training=self.training)
        ...
        x = x.transpose(0, 1)
        # compute padding mask
        encoder_padding_mask = srcz_tokens.eq(self.padding_idx)
        encoder_padding_mask = torch.cat([encoder_padding_mask.new(srcz_tokens.size(0), 1).fill_(0), encoder_padding_mask], dim=1)
        if not encoder_padding_mask.any():
            encoder_padding_mask = None
        ...
        return {
            'encoder_z_out': x,  # T x B x C
            'encoder_z_padding_mask': encoder_padding_mask,  # B x T
        }
    ...
    def reorder_encoder_out(self, encoder_out, new_order):
        ...
        Returns:
            *encoder_out* rearranged according to *new_order*
        """
        if encoder_out['encoder_z_out'] is not None:
            encoder_out['encoder_z_out'] = \
                encoder_out['encoder_z_out'].index_select(1, new_order)
        if encoder_out['encoder_z_padding_mask'] is not None:
            encoder_out['encoder_z_padding_mask'] = \
                encoder_out['encoder_z_padding_mask'].index_select(0, new_order)
...
@register_model_architecture('bert_transformer_seq2seq_pinyin', 'bert_transformer_seq2seq_pinyin')   #args是一个对象，getattr用来获取对象中的属性值
...
@register_model_architecture('bert_transformer_seq2seq_pinyin', 'bert_transformer_seq2seq_big_pinyin')
