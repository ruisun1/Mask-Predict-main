...
@register_model('bert_transformer_seq2seq_pretrained')
class Transformer_nonautoregressive(FairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        def build_embedding(dictionary, embed_dim, is_encoder, path=None):
            ...
            config = BertConfig.from_pretrained('/home/sunrui/Mask-Predict-main/Realise')
            emb = BertModel.from_pretrained('/home/sunrui/Mask-Predict-main/Realise',config = config)
        ...
        encoder_x = TransformerEncoder_x(args, src_dict, encoder_embed_tokens, args.encoder_embed_scale)
        encoder_z = TransformerEncoder_z(args, src_dict, encoder_embed_tokens, args.encoder_embed_scale)
...
class SelfTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_x_out=None, encoder_z_out=None, incremental_state=None):
       ...
       # print("decoder_padding_mask",decoder_padding_mask.size())
        ...
        x = self.embed_tokens(prev_output_tokens)[0]   # from build embeddings
        ...
        if self.adaptive_softmax is None and self.load_softmax:
            ...
            if self.share_input_output_embed:
                #print(x.size(),self.embed_tokens.embeddings.word_embeddings.weight.size())
                x = F.linear(x, self.embed_tokens.embeddings.word_embeddings.weight)
            else:
...
class TransformerEncoder_x(FairseqEncoder):
    ...
    def forward(self, srcx_tokens, srcx_lengths):
        ...
        """
      #  print(srcx_tokens)
      #  print(self.dic.string(srcx_tokens))
        #print("src_tokens size",srcx_tokens)
        # embed tokens and positions
        x = self.embed_tokens(srcx_tokens)[0]
        ...
        return {
            ...
            'encoder_x_padding_mask': encoder_padding_mask,  # B x
        }
...
class TransformerEncoder_z(FairseqEncoder):
    ...
    def forward(self, srcz_tokens, srcz_lengths):
        ...
        # embed tokens and positions
        x = self.embed_tokens(srcz_tokens)[0]
        #assert (src_tokens.size(1) < self.embed_positions.weights.data.size(0))
        if self.embed_positions is not None:
...
@register_model_architecture('bert_transformer_seq2seq_pretrained', 'bert_transformer_seq2seq_pretrained')   #args是一个对象，getattr用来获取对象中的属性值
...
@register_model_architecture('bert_transformer_seq2seq_pretrained', 'bert_transformer_seq2seq_pretrained_big')
