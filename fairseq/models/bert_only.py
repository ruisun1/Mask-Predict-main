...
@register_model('bert_1encoder_only')
class Transformer_nonautoregressive(FairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        encoder_x = TransformerEncoder_x(args, src_dict, encoder_embed_bert,  bert_config)
        decoder = SelfTransformerDecoder(args, tgt_dict, decoder_embed_bert,  bert_config)
...
class SelfTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_bert,  bert_config,  embed_scale=None, no_encoder_attn=False, left_pad=False,
                 final_norm=True):
        ...
        self.map_fc = nn.Linear(bert_config.hidden_size  , bert_config.vocab_size)   #p
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
     ...
     #batch, seq_len, hiddensize*2
        x = self.map_fc(context_bert_output)
        bsz ,seq, vol = x.size()
        y = x.view(-1, x.size(-1))
        y = F.softmax(x, -1)
...
class TransformerEncoder_x(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_bert,  bert_config, embed_scale=None, left_pad=False):
        super().__init__(dictionary)
        self.dropout = args.dropout
        self.bert_model = embed_bert
        self.padding_idx = dictionary.pad()
        ...
        self.embed_scale = math.sqrt(args.encoder_embed_dim) if embed_scale is None else embed_scale
        self.map_fc = nn.Linear(bert_config.hidden_size , bert_config.hidden_size)   #p
        self.dropout = nn.Dropout(bert_config.hidden_dropout_prob)
    ...
    def forward(self, srcx_tokens, srcx_lengths):
        ...
        last_hidden_state = self.map_fc(context_bert_output)
        return {
...
@register_model_architecture('bert_1encoder_only', 'bert_1encoder_only')   #args是一个对象，getattr用来获取对象中的属性值
...
@register_model_architecture('bert_1encoder_only', 'bert_1encoder_only_big')
