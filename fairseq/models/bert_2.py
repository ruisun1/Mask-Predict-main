...
@register_model('bert_model_seq2seq')
class Transformer_nonautoregressive(FairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        config = BertConfig.from_pretrained('/home/sunrui/bert-base-chinese/bert_config.json')
        bert = BertModel.from_pretrained('/home/sunrui/bert-base-chinese/pytorch_model.bin', config=config)
        ...
        encoder_x = TransformerEncoder_x(args, src_dict, encoder_embed_tokens, bert, args.encoder_embed_scale)
        encoder_z = TransformerEncoder_z(args, src_dict, encoder_embed_tokens, bert, args.encoder_embed_scale)
        decoder = SelfTransformerDecoder(args, tgt_dict, decoder_embed_tokens, bert, args.decoder_embed_scale)
...
class SelfTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, bert, embed_scale=None, no_encoder_attn=False, left_pad=False,
                 final_norm=True):
    ...
    def forward(self, prev_output_tokens, encoder_x_out=None, encoder_z_out=None, incremental_state=None):
       ...
       # segments_ids=prev_output_tokens.new(prev_output_tokens.size())._fill(0)
        x= self.bert(prev_output_tokens , attention_mask=  decoder_padding_mask)[0] ## bsz, seq_len, hidden_state
        x = x.transpose(0, 1)
...
class TransformerEncoder_x(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, bert ,embed_scale=None, left_pad=False):
        super().__init__(dictionary)
        self.dropout = args.dropout
        self.bert= bert
    ...
    def forward(self, srcx_tokens, srcx_lengths):
        ...
        #print("srcx_tokens",srcx_tokens.size())
       # srcx_tokens  = torch.LongTensor([item.cpu().detach().numpy() for item in srcx_tokens]).cuda()
       # srcx_lengths = torch.LongTensor([item.cpu().detach().numpy() for item in srcx_lengths]).cuda()
       ...
       # encoder_padding_mask = torch.cat([encoder_padding_mask.new(srcx_tokens.size(0), 1).fill_(0), encoder_padding_mask], dim=1)
        ...
        #segments_ids=srcx_tokens.new(srcx_tokens.size()).fill_(0)
        # encoder layers
        #print("srcx_tokens",srcx_tokens)
        #print("srcx size ,lensize",srcx_tokens.size(),encoder_padding_mask.size())
        x= self.bert(srcx_tokens , attention_mask =  encoder_padding_mask)[0] ## bsz, seq_len, hidden_state
        x = F.dropout(x, p=self.dropout, training=self.training)
        x= x.transpose(0,1)
        for layer in self.layers:
...
class TransformerEncoder_z(FairseqEncoder):
    ...
    def forward(self, srcz_tokens, srcz_lengths):
        ...
        # embed tokens and positions
        #print("srcz_token",srcz_tokens.size())
        #srcz_tokens  = torch.LongTensor([item.cpu().detach().numpy() for item in srcz_tokens]).cuda()
        #srcz_lengths = torch.LongTensor([item.cpu().detach().numpy() for item in srcz_lengths]).cuda()
       ...
       # segments_ids=srcz_tokens.new(srcz_tokens.size()).fill_(0)
         # encoder layers
        x= self.bert(srcz_tokens , attention_mask=  encoder_padding_mask)[0] ## bsz, seq_len, hidden_state
        x = F.dropout(x, p=self.dropout, training=self.training)
        x= x.transpose(0,1)
        for layer in self.layers:
            x= layer(x, encoder_padding_mask)
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_padding_mask):
        ...
        Returns:
            encoded output of shape `(batch, src_len, embed_dim)`
        """
        #print("x,encoder_padding_mask",x.size(),encoder_padding_mask.size())
        residual = x
...
@register_model_architecture('bert_model_seq2seq', 'bert_model_seq2seq')   #args是一个对象，getattr用来获取对象中的属性值
...
@register_model_architecture('bert_model_seq2seq', 'bert_model_seq2seq_big')
