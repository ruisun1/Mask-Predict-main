...
@register_strategy('mask_predict_1encoder')
class MaskPredict1E(DecodingStrategy):
    
    def __init__(self, args):
        super().__init__()
        self.iterations = args.decoding_iterations
    
    def generate(self, model, encoder_x_out,  tgt_tokens, tgt_dict):
        ...
        tgt_tokens, token_probs = self.generate_non_autoregressive(model, encoder_x_out,  tgt_tokens)
        ...
        for counter in range(1, iterations):
           ...
           # print("Masking: ", convert_tokens(tgt_dict, tgt_tokens[0]))
            decoder_out = model.decoder(tgt_tokens, encoder_x_out )
    ...
    def generate_non_autoregressive(self, model, encoder_x_out,  tgt_tokens):
        decoder_out = model.decoder(tgt_tokens, encoder_x_out)
