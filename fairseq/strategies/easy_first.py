...
from .strategy_utils import duplicate_encoder_out1, duplicate_encoder_out2,  generate_step_with_prob, assign_single_value_long, assign_single_value_byte, assign_multi_value_long, convert_tokens
...
class EasyFirst(DecodingStrategy):
    ...
    def generate(self, model, encoder_x_out, encoder_z_out, tokens, tgt_dict):
        bsz, seq_len = tokens.size()
        duplicate_encoder_out1(encoder_x_out, bsz, self.beam_size)
        duplicate_encoder_out2(encoder_z_out, bsz, self.beam_size)
        ...
        while tokens.eq(tgt_dict.mask()).sum() > 0:
            ...
            decoder_out = model.decoder(tokens, encoder_x_out,encoder_z_out)
