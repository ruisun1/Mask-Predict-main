...
@register_strategy('mask_predict_mLMfilter')
class MaskPredict_mLMfilter(DecodingStrategy):
    ...
    def generate(self, model, encoder_x_out, encoder_z_out,  tgt_tokens, tgt_dict):
        ...
        mask_tokens = tgt_tokens.new(tgt_tokens.size()).fill_(tgt_dict.mask())
        assign_single_value_byte(mask_tokens, pad_mask, tgt_dict.pad())
        #iterations = seq_len if self.iterations is None else self.iteration
        new_tgt_tokens, new_token_probs = self.generate_non_autoregressive(model, encoder_x_out,encoder_z_out, mask_tokens)
        #print("nat",new_token_probs.size())
        assign_single_value_byte(new_tgt_tokens, pad_mask , tgt_dict.pad())
        different = (tgt_tokens != new_tgt_tokens)
        print(different)
        assign_single_value_long(new_tgt_tokens, different, tgt_dict.mask())
...
#        assign_single_value_long(new_tgt_tokens, pad_mask, tgt_dict.pad())
        ...
        tgt_tokens , token_probs = new_tgt_tokens, new_token_probs
