...
from fairseq.strategies.strategy_utils import duplicate_encoder_out1, duplicate_encoder_out2
...
def main(args):
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        f1=open('./gold.txt','w',encoding='utf-8')
        f2=open('./pre.txt','w',encoding='utf-8')
        f3=open('./wro.txt','w',encoding='utf-8')
        ...
        #    print("src_tokens",src_tokens)
            ...
            if not args.quiet:
                ...
                f3.writelines(src_str+'\n')
                if has_target:
                    print('T-{}\t{}'.format(sample_id, target_str))
                    f1.writelines(target_str+'\n')
...
def generate_batched_itr(data_itr, strategy, models, tgt_dict, length_beam_size=None, use_gold_target_len=False, cuda=True):
    ...
    for sample in data_itr:
       # s=sample
        ...
        #print("decoder_src",tgt_dict.string(decoder_src['prev_output_tokens']))
        with torch.no_grad():
            gold_target_len = s['target'].ne(tgt_dict.pad()).sum(-1) if use_gold_target_len else None
            hypos = generate(s['target'],strategy, encoder_input, models, tgt_dict, length_beam_size, gold_target_len, decoder_src)
            for batch in range(hypos.size(0)):
                src = utils.strip_pad(input['srcx_tokens'][batch].data, tgt_dict.pad())
...
def generate(target, strategy, encoder_input, models, tgt_dict, length_beam_size, gold_target_len, decoder_src):
    assert len(models) == 1
    model = models[0]
    
    src_tokens = encoder_input['srcx_tokens']
    src_tokens = src_tokens.new(src_tokens.tolist())
    #print(src_tokens.size())
    bsz = src_tokens.size(0)
    #print(encoder_input) 
    encoder_x_out = model.encoder(encoder_input['srcx_tokens'],encoder_input['srcx_lengths'])
   # print(beam)
   # max_len = beam.max().item()
   # print("max_len",max_len)
    length_mask = target.ne(tgt_dict.pad())# bsz, max_len
    max_len = src_tokens.size(1)
    tgt_tokens = decoder_src['prev_output_tokens']
    #print(tgt_tokens)
    tgt_tokens =  length_mask * tgt_tokens + ~length_mask * tgt_dict.pad()
    #print(tgt_tokens)
    #tgt_tokens = tgt_tokens.view(bsz * length_beam_size, max_len)
 
    #encoder_z_out = model.encoderz(encoder_input['srcz_tokens'],encoder_input['srcz_lengths'])
   # duplicate_encoder_out1(encoder_x_out, bsz, length_beam_size)
   # duplicate_encoder_out2(encoder_z_out, bsz, length_beam_size)
    hypotheses, lprobs = strategy.generate(model, encoder_x_out,  tgt_tokens, tgt_dict)
    
    hypotheses = hypotheses.view(bsz,  max_len)
