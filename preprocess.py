...
from fairseq.data import indexed_dataset, dic2
...
def main(args):
    ...
    def build_dictionary(filenames):
        ...
        for filename in filenames:
            dic2.Dictionary().add_file_to_dictionary(filename, d, tokenize_line, args.workers)
        return d
...
#    tgt_dict = dictionary.Dictionary.load(args.tgtdict)
    if args.joined_dictionary:
        ...
        if args.srcdict:
            src_dict = dic2.Dictionary.load(args.srcdict)
        else:
            assert args.trainpref, "--trainpref must be set if --srcdict is not specified"
            src_dict = build_dictionary([train_path(args.source_lang)])
            #src_dict = build_dictionary([train_path(args.
        if target:
            if args.tgtdict:
                tgt_dict = dic2.Dictionary.load(args.tgtdict)
            else:
    ...
    if args.alignfile:
        ...
        src_dict = dic2.Dictionary.load(dict_path(args.source_lang))
        tgt_dict = dic2.Dictionary.load(dict_path(args.target_lang))
        freq_map = {}
